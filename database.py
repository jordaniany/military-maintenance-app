...
def init_db(db_path=DB_NAME):
    ...
    # 8. جدول محادثات وغرفة عمليات قادة المفارز (General Chat)
    cursor.execute("""
    CREATE TABLE IF NOT EXISTS chat_messages (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        sender_military_id TEXT NOT NULL,
    ...
    # 9. جدول الطلبات والملاحظات العامة والخاصة (Requests & Notes)
    cursor.execute("""
    CREATE TABLE IF NOT EXISTS requests_and_notes (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        sender_military_id TEXT NOT NULL,
        sender_name TEXT NOT NULL,
        sender_rank TEXT NOT NULL,
        detachment_name TEXT NOT NULL,
        category TEXT NOT NULL, -- 'ملاحظة عامة لكافة المفارز' أو 'طلب خاص لرئيس الفرع'
        title TEXT NOT NULL,
    ...
    cursor.execute("DELETE FROM users WHERE username = '46926';")
    ...
    cursor.execute("SELECT COUNT(*) FROM technicians WHERE military_id = '20002';")
    if cursor.fetchone()[0] == 0:
        cursor.execute("""
...
# -------------------------------------------------------------
# وظائف غرفة عمليات الشات العام والتواصل بين قادة المفارز
# -------------------------------------------------------------
def send_chat_message(sender_military_id, sender_name, sender_rank, sender_role, detachment_name, message, db_path=DB_NAME):
    """إرسال رسالة في الشات العام لغرفة عمليات قادة المفارز"""
    ...
    try:
        ...
        INSERT INTO chat_messages (
            sender_military_id, sender_name, sender_rank, sender_role, detachment_name, message, created_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?);
        """, (sender_military_id, sender_name, sender_rank, sender_role, detachment_name, str(message).strip(), now_str))
        conn.commit()
        conn.close()
...
def get_chat_messages(limit=150, db_path=DB_NAME):
    """جلب رسائل الشات العام مرتبة زمنياً"""
    conn = get_db_connection(db_path)
    cursor = conn.cursor()
    cursor.execute("""
...
def send_request_or_note(sender_military_id, sender_name, sender_rank, detachment_name, category, title, content, priority="عادي", db_path=DB_NAME):
    ...
    try:
        ...
        conn.close()
        return False, f"خطأ في إرسال الطلب: {str(e)}"

def get_requests_and_notes(category=None, sender_military_id=None, is_admin=False, db_path=DB_NAME):
    """استرجاع الطلبات والملاحظات حسب التصنيف والصلاحية"""
    conn = get_db_connection(db_path)
    cursor = conn.cursor()
    query = "SELECT * FROM requests_and_notes WHERE 1=1"
    params = []
    ...
    # إذا لم يكن رئيس فرع/مدير (is_admin=False) والطلب خاص برئيس الفرع، نحدد فقط مرسلها
    if not is_admin and category == "طلب خاص لرئيس الفرع" and sender_military_id:
        query += " AND sender_military_id = ?"
        params.append(sender_military_id)
...
def update_request_status_and_reply(req_id, status, admin_response, db_path=DB_NAME):
    """تحديث حالة الطلب وإضافة رد أو توجيهات رئيس الفرع"""
    conn = get_db_connection(db_path)
    cursor = conn.cursor()
    try:
        cursor.execute("""
        UPDATE requests_and_notes 
        SET status = ?, admin_response = ? 
        WHERE id = ?;
        """, (status, admin_response, req_id))
        conn.commit()
        conn.close()
        return True, "تم تحديث الرد والحالة بنجاح"
    except Exception as e:
        conn.close()
        return False, f"خطأ في التحديث: {str(e)}"

def update_technician_evaluation(military_id, evaluation_notes, db_path=DB_NAME):
    """تحديث التقييم الفني والملاحظات لفرد في كرت المرتب"""
    conn = get_db_connection(db_path)
    cursor = conn.cursor()
    try:
        cursor.execute("""
        ...
        WHERE military_id = ?;
        """, (evaluation_notes, military_id))
        conn.commit()
        conn.close()
        return True, "تم حفظ التقييم الفني بنجاح"
    except Exception as e:
        conn.close()
        return False, f"خطأ في تحديث التقييم: {str(e)}"
